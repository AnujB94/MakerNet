...
**External inputs required for release acceptance:** College IT must provide and register the production OIDC issuer, client, verified institution-email claim, and callback, plus TLS PostgreSQL runtime and migration access. Configure those values directly in Vercel; no credentials belong in this repository or chat. Run an actual college-SSO sign-in, account-state and role matrix, live database readiness, and manual screen-reader smoke test. Until these pass, the dynamic Vercel site is not accepted as a functioning release. Local Docker staging and automated checks are recorded in the subsystem checklists below.
