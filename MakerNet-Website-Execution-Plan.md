...
**Subsystem 1 open gate:** Manual screen-reader verification could not run because this environment exposed neither a visible browser through Browser Use nor the Windows Computer Use native helper. The candidate remains a draft pull request; merge and `v0.2.0` tagging await this final check. Keep Subsystem 2 closed until the Subsystem 1 gate is met.
